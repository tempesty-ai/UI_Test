...
이 결과를 "신뢰도 수치"로 읽으면 안 된다. n=4는 통계적으로 무의미하며,
이 실행의 목적은 정확도 측정이 아니라 **실패 모드 탐색**이었다.

| 항목 | 측정 여부 | 근거 |
|---|---|---|
| False Positive | ✅ 측정 | 에이전트가 낸 이슈 4건을 전수 재검토 (FP 2건 확인) |
| False Negative | ❌ **미측정** | PASS 판정 항목을 사람이 전수 재검사해야 산출 가능 |
| 판정 일치율 | △ 참고치 | n=4로는 신뢰구간을 말할 수 없음 |

**"보수적으로 동작했다"는 관찰이지 검증이 아니다.**
이슈 4건 중 2건이 FP였다는 점은 에이전트가 과잉 판정 쪽으로 기울었음을 시사하지만,
FN을 직접 재지 않은 이상 "놓침이 적다"는 결론은 낼 수 없다.
FP가 높다고 FN이 낮다는 보장은 없다.

### FN 측정을 하려면 (미구현 설계)

1. PASS 판정 항목을 카테고리별로 **층화 샘플링** (전수는 비용상 불가)
2. 샘플을 사람이 blind 재검사 — 에이전트 판정을 보지 않은 상태로
3. 샘플 FN율로 전체 추정 + 신뢰구간 산출
4. 반응형·콘솔 에러처럼 **FN이 몰릴 것으로 예상되는 카테고리는 샘플 비중 상향**
