...
이 프로젝트의 핵심 차별점입니다. 에이전트가 낸 판정을 **그대로 믿지 않고 사람이 재검토**해 신뢰도를 측정합니다.
이것이 "에이전트를 쓰는 것"과 "에이전트 출력을 QA하는 것"의 차이입니다.

> ⚠️ 아래 수치는 예시입니다. 실제 재검토 결과로 교체하세요.

| 항목                       | 수치          |
| -------------------------- | ------------- |
| 심각도(P1~P3) 판정 일치율  | [18/20 (90%)] |
| False Positive (과잉 이슈) | [2건]         |
| False Negative (놓친 이슈) | [0건]         |

> 놓침(FN)이 [0]인 대신 과잉 판정(FP)이 있어, **릴리즈 안전 쪽으로 보수적으로 튜닝**되었음을 확인.
> Tool Call(브라우저 조작) 실패 시 재시도 로직을 추가해 FP를 줄이는 것이 다음 개선 과제입니다.

에이전트를 신뢰할 수 있는가는 감이 아니라 이 지표로 이야기합니다. 하네스가 "행동"을 통제한다면, 이 재검증 단계는 "판정"을 통제합니다.
