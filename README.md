...
이 표는 "에이전트 신뢰도가 몇 %다"를 말하는 게 아니다.
4건은 통계를 낼 수 있는 양이 아니고, 이번 실행의 목적도
정확도 측정이 아니라 **에이전트가 어떤 식으로 틀리는지 찾는 것**이었다.

**잰 것 — 헛짚음(False Positive)**
에이전트가 보고한 이슈 4건을 전부 다시 확인해서 2건이 오판임을 확인했다.
보고된 건만 보면 되므로 측정 가능했다.

**못 잰 것 — 놓침(False Negative)**
에이전트가 "이상 없음"으로 넘긴 항목 중에 진짜 결함이 있었는지는 확인하지 못했다.
이걸 재려면 PASS 판정 항목 전체를 사람이 다시 검사해야 하는데, 비용이 커서 하지 못했다.

**그래서 "놓침이 적다"고 말할 수 없다.**
헛짚음이 많았다는 건 에이전트가 과하게 잡아내는 쪽으로 기울었다는 *정황*이지만,
헛짚음이 많다고 놓침이 적다는 보장은 없다. 두 값은 따로 움직인다.

### 놓침을 재려면 (아직 안 함)

전수 검사는 비용상 불가능하므로 샘플링으로 접근한다.

1. PASS 판정 항목 중 일부를 뽑는다 — 카테고리별로 골고루
2. 사람이 다시 검사한다. 이때 **에이전트 판정은 보지 않는다** (선입견 차단)
3. 샘플에서 나온 놓침 비율로 전체를 추정한다
4. 반응형·콘솔 에러처럼 놓치기 쉬운 영역은 샘플을 더 많이 뽑는다
